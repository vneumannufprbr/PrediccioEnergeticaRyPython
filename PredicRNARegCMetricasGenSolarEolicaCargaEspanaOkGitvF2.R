...
for(target_var in targets) {
  ...
  for(i in 1:test_size) {
    ...
    # *** INICIO CORRECCIÓN 2: Validar la predicción ***
  ...
  # *** INICIO CORRECCIÓN 1 (para datos completos) ***
  ...
  for(i in 1:forecast_horizon) {
    ...
    # *** INICIO CORRECCIÓN 2 (para datos completos) ***
    if (!is.finite(pred_actual)) {
      pred_actual <- 0
    }
    # *** FIN CORRECCIÓN 2 ***
...
forecast_df %>%
  ...
  labs(title = "Pronóstico a 30 días usando Redes Neuronales (RNA)",
       x = "Fecha", y = "Valor") +
  theme_minimal()
